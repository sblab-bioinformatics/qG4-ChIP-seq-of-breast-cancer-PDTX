---
title: "Characterization of DG4R and CG4R"
output: github_document
author: angela simeone
---


This document illustrates steps followed to characterize ΔG4r previously identified.

## Pairwise comparison of PDTXs ΔG4r by computing Jaccard index
```{bash, eval = F}
...
```{bash, eval = F}
pdtx_path=~/PDTX_dm6_inputSubtraction_
breast_cancer_driver=~/regions_driver_breastcancer.hg19.sort.bed
cases=(DG4R_CG4R)
for case in ${cases[@]}
do
...
awk '{print $0"\tBREAST_DRIV"}' ~/regions_driver_breastcancer.hg19.sort.bed > ~/annotation_regions_driver_breastcancer.hg19.sort.bed

workspace_gen=~/reference_genomes/hg38/hg38.whitelist.bed
anno=~/annotation_regions_driver_breastcancer.hg19.sort.bed
for case in ${cases[@]}
do
  cd $pdtx_path/$case
  out_folder=$pdtx_path/$case/DG4R/intervene_DG4r_CG4r_breast_cancer_drivers/
...
library(ggplot2)
setwd("/Users/simeon01/Documents/PDTX/REVISIONS/PDTX_for_figures_generation/PDTX_dm6_inputSubtraction_/DG4R/intervene_DG4r_CG4r_breast_cancer_drivers")
...
After identifications of DG4R, generate CG4R.
analyze the G4 motifs prevalence and enrichments at DG4R regions.
Steps:
...
Rscript barplot_N_peaks.R ~/PDTX_dm6_inputSubtraction_

# after moving the files on the cluster
### generate folder with DG4R and produce permuations of the same file
consensus_hg19=~/fastq/trimmed/aligned/merged_lines/consensus_hg19/hg19_old_new_q005.all_peaks.25M.over99nt.sorted.bed

path_DG4R_CG4R=~/PDTX_dm6_inputSubtraction_/DG4R_CG4R

g=~/reference_genomes/robert_genome/hg19.size.genome
hg19_white=~/reference_genomes/robert_genome/hg19.whitelist.bed
fasta=~/reference_genomes/hg19_gen/hsa.hg19.fa
...
g=~/reference_genomes/robert_genome/hg19.size.genome
fasta=~/reference_genomes/hg19_gen/hsa.hg19.fa
cd /scratchb/sblab/simeon01/PDTX_dm6_inputSubtraction_/DG4R_CG4R

outout_fasta_dir=/scratchb/sblab/simeon01/PDTX_dm6_inputSubtraction_/DG4R_CG4R/hg19_old_new_q005_fasta
...
The R scripts [__Association DG4r to expression groups__](./Compare_expressionValues_DG4R_CG4R.R) perform the analyisis on the association of DG4r to highly expressed genes by PDTX. 

```{bash, eval = F}
# run this locally

promoter=~/Annotation_robert/hg19.gene_name.promoters.bed
path_analysis=~/PDTX_dir
cases=(PDTX_dm6_inputSubtraction_)
...
original: Analysis_PDTX_NO_cells_new_old_PDTX.*